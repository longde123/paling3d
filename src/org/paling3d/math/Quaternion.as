package  org.paling3d.math
{
	...
	public class Quaternion 
	{
		 ...
		 */
		public static function createFromMatrix( matrix:Matrix4x4 ):Quaternion
		{
			...
			var tr:Number = matrix._11 + matrix._22 + matrix._33;
			...
			if (tr > 0.0) 
			{
				...
				quat.x = (matrix._32 - matrix._23) * s;
				quat.y = (matrix._13 - matrix._31) * s;
				quat.z = (matrix._21 - matrix._12) * s;
			} 
			else 
			{		
		 ...
		 * @return  the created Quaternion
		 */
		public static function createFromOrthoMatrix( m:Matrix4x4 ):Quaternion
		{
			var q:Quaternion = new Quaternion();
			...
			// recover signs
			q.x = m._32 - m._23 < 0 ? (q.x < 0 ? q.x : -q.x) : (q.x < 0 ? -q.x : q.x);
			q.y = m._13 - m._31 < 0 ? (q.y < 0 ? q.y : -q.y) : (q.y < 0 ? -q.y : q.y);
			q.z = m._21 - m._12 < 0 ? (q.z < 0 ? q.z : -q.z) : (q.z < 0 ? -q.z : q.z);
		...
		public function get matrix():Matrix4x4
		{
			...
			_matrix._11 = 1 - 2 * ( yy + zz );
			_matrix._12 =     2 * ( xy - zw );
			_matrix._13 =     2 * ( xz + yw );
			
			_matrix._21 =     2 * ( xy + zw );
			_matrix._22 = 1 - 2 * ( xx + zz );
			_matrix._23 =     2 * ( yz - xw );
			
			_matrix._31 =     2 * ( xz - yw );
			_matrix._32 =     2 * ( yz + xw );
			_matrix._33 = 1 - 2 * ( xx + yy );
